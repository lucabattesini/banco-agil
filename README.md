...
- **Redirecionamento direto entre especialista e Triagem, implementado mas não oferecido a nenhum agente** — a tool `return_to_triage` permanece implementada e testada, mas nenhum agente a recebe hoje — todo roteamento passa pela Triagem a cada mensagem. Com mais tempo para refinar em que condições é seguro delegar sem reabrir risco de loop, é uma boa opção futura de otimização de tokens e tempo de resposta.
